...
export default function ProfessorAssignment() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !title ||
      !grade ||
      (grade === "select" && !manualGrade) ||
      !instructions ||
      (dueDateOption === "calendar" && !dueDate) ||
      !pdfDocument
    ) {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3000/uploadAssignment",
        formData,
        {
  ...
  return (
    <>
      <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-50 p-6">
        <ToastContainer />
